...
def show_account_menu():
    ...
    while in_account_menu:
        ...
        if AuthInstance.get_active_user() is None or add_user:
            ...
            # Optionally ask for a friendly name after adding
            name_input = cyber_input("Masukkan nama untuk akun ini (opsional)")
            if name_input.strip() != "":
                # Find the just-added entry and set name, then save
                for rt in AuthInstance.refresh_tokens:
                    if rt.get("number") == int(number):
                        rt["name"] = name_input.strip()
        ...
        if not users or len(users) == 0:
            ...
            for idx, user in enumerate(users):
                ...
                name = user.get("name", "")  # New name field (optional)
                sub_type = user.get("subscription_type", "")

                table.add_row(str(idx + 1), number, name, sub_type, active_marker)
...
[bold white]name <N>[/]: Tambah / Edit Nama Akun
[bold white]00[/]: Kembali ke menu utama""",
        ...
        if input_str == "00":
            ...
            if len(parts) == 2 and parts[1].isdigit():
                ...
                pause()
            continue
        elif input_str.startswith("name "):
            parts = input_str.split()
            if len(parts) == 2 and parts[1].isdigit():
                ...
                console.print("[error]Perintah tidak valid. Gunakan format: name <nomor urut>[/]")
                pause()
            continue
        else:
